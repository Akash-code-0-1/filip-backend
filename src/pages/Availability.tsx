...
import { fetchJobs } from "../store/features/availabilitySlice";
import type { RootState, AppDispatch } from "../store"; // <-- import RootState and AppDispatch
 // <-- import RootState and AppDispatch

const currentlyWorking = [
  {
    name: "Marcus Chen",
    workplace: "At: Sky Longue Resturant",
...
export default function Availability() {
  ...
  const { calendar, loading, availableNow, skillCounts } = useSelector(
    (state: RootState) => state.availability // <-- typed state
  );
  ...
  useEffect(() => {
    dispatch(fetchJobs()); // now TypeScript will not complain
  }, [dispatch]);
            ...
            <div className="bg-[#1f1f1f] border border-[#2a2a2a] rounded-xl p-4 sm:p-5">
              ...
              <div className="space-y-3">
                {availableNow.map((worker, index) => (
                  <div
                    key={index}
                    className="flex items-center justify-between p-3 bg-[#2a2a2a]/50 rounded-lg border-l-2 border-[#4CAF50]"
                  >
                    <div>
